...
proc install*(
    project: Project,
    dontInstall, dontHandleUsers, dontHandleScripts, dontHandleDotfiles, dontHandleHome:
      bool,
) =
  ...
  if not dontInstall:
    ...
    let
      res =
        execCmdEx("sudo install -Dm755 " & getAppFilename() & " /usr/bin/districity")
